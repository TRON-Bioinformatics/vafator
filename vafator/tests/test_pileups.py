...
import pysam
from vafator.pileups import get_insertion_metrics, get_variant_pileup, VafatorVariant, get_snv_metrics, \
...
class TestPileups(TestCase):
    ...
    def test_snv_metrics(self):
        ...
        self._assert_snv(variant=variant, expected_ac={'G': 5, 'T': 6}, expected_dp=11)
        self._assert_snv(variant=variant, expected_ac={'G': 1, 'T': 2}, expected_dp=3, min_base_quality=40)

    def test_snv_metrics_2(self):
        variant = VafatorVariant(chromosome="chr1", position=1018144, reference="T", alternative=["C"])
        self._assert_snv(variant=variant, expected_ac={'C': 9, 'T': 11}, expected_dp=20)
        self._assert_snv(variant=variant, expected_ac={'C': 3, 'T': 4}, expected_dp=7, min_base_quality=40)
        self._assert_snv(variant=variant, expected_ac={'C': 0, 'T': 0}, expected_dp=0, min_mapping_quality=65)

    def _assert_snv(self, variant: VafatorVariant, expected_ac, expected_dp,
        ...
        self.assertEqual(expected_ac, ac)
        self.assertEqual(expected_dp, dp)
    ...
    def test_insertion_metrics(self):
        ...
        self._assert_insertion(variant=variant, expected_ac=0, expected_dp=3)
        ...
        self._assert_insertion(variant=variant, expected_ac=1, expected_dp=3)
    ...
    def test_insertion_metrics_2(self):
        ...
        self._assert_insertion(variant=variant, expected_ac=9, expected_dp=11)
        self._assert_insertion(variant=variant, expected_ac=5, expected_dp=7, min_mapping_quality=40)
        self._assert_insertion(variant=variant, expected_ac=4, expected_dp=4, min_base_quality=40)

    def _assert_insertion(self, variant: VafatorVariant, expected_ac, expected_dp,
                    min_base_quality=0, min_mapping_quality=0):
        pileups = get_variant_pileup(
            variant=variant, bam=self.bam_reader,
            min_base_quality=min_base_quality, min_mapping_quality=min_mapping_quality)
    ...
    def test_deletion_metrics(self):
        ...
        self._assert_deletion(variant=variant, expected_ac=12, expected_dp=13)
        self._assert_deletion(variant=variant, expected_ac=0, expected_dp=0, min_mapping_quality=61)
        self._assert_deletion(variant=variant, expected_ac=3, expected_dp=3, min_base_quality=40)
    ...
    def _assert_deletion(self, variant: VafatorVariant, expected_ac, expected_dp,
                    min_base_quality=0, min_mapping_quality=0):
        pileups = get_variant_pileup(
            variant=variant, bam=self.bam_reader,
            min_base_quality=min_base_quality, min_mapping_quality=min_mapping_quality)
