...
output=output/integration_test_06

input_ucn=$test_data/best.seg.minimal.ucn

hatchet2bed --input-file $input_ucn --output-prefix test_prefix


input_vcf=$test_data/project.NIST.hc.snps.indels.chr1_1000000_2000000.vcf
input_bed=test_prefix.my_tumor.bed
...
--purity my_tumor 0.5 --tumor-ploidy my_tumor $input_bed --normal-ploidy 2
...
assert_eq `cat $output_vcf | grep -v '#' | grep my_normal_pw | wc -l` `cat $input_vcf | grep -v '#' | wc -l` "Wrong number of variants"
assert_eq `cat $output_vcf | grep -v '#' | grep my_tumor_pw | wc -l` `cat $input_vcf | grep -v '#' | wc -l` "Wrong number of variants"
